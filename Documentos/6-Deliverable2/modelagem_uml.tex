...
%  \item A estação base deve receber informações de obstáculos detectados, processá-las e armazená-las na memória. Representado pelo requisito funcional: \textbf{"Estação base mostra na interface gráfica os novos obstáculos detectados pelo robô - RF\arabic{enumi}"}
  ...
  \item O usuário pode salvar o mapa 2D no disco rígido. Representado pelo requisito funcional: \textbf{``O usuário pode salvar o mapa -- RF\arabic{enumi}''}.
...
O \textit{software} da estação base do robô foi dividido nos seguintes pacotes:  \textit{visual}, \textit{dados}, \textit{comunicacao}, e \textit{gui}. A seguir há uma descrição de cada pacote e das suas respectivas classes.
...
A Figura \ref{fig:diagrama_classes_sist_embarcado} mostra o diagrama de classes do sistema embarcado (placa TS-7260). A descrição das classes encontra-se na Tabela \ref{tab:descricao_sistema_embarcado}.
\begin{figure}[H]
  \centering
...
\begin{table}[H]
  ...
  \begin{tabular}{p{6cm}p{8cm}}
   ...
   SensorsSampler & Thread responsável por requisitar amostras dos sensores da placa de baixo nível em intervalos de tempo previamente programados. \\ \hline
   ...
   SerialCommunicator & Responsável por gerenciar a comunicação via porta serial entre a TS-7260 e a LPC2103.\\
    \bottomrule
  \end{tabular}%
  \label{tab:descricao_sistema_embarcado}%
\end{table}%
